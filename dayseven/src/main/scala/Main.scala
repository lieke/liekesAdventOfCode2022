...
@main def daySeven2022: Unit = {
  val initTime = System.currentTimeMillis
  println("Welcome to day seven of the Advent of Code!")
  val directoryStructure = new DirectoryStructure(input)
  val totalSizesOfSmallerDirectories = directoryStructure.findFilesWithAtMostCertainSize(100000).sum
  println("The sum of the total directories with at most 100000 is " + totalSizesOfSmallerDirectories)
  val sizeOfSmallestDirectoryThatFreesUpEnoughSpace = directoryStructure.findSizeOfSmallestDirectoryThatFreesUpEnoughSpace()
  println("The size of the smallest directory that frees up enough space is " + sizeOfSmallestDirectoryThatFreesUpEnoughSpace)
...
class DirectoryStructure() {
  ...
  var allFiles = new HashMap[String, File]()
  var currentDir = new File("/", 0, true)
  allFiles.addOne("/", currentDir)

  def this(input: List[String]) = {
    this()
    input.foreach(parseFileInputString(_))
  }

  def parseFileInputString(input: String): Unit = {
    input match {
      case "cd .." => currentDir = currentDir.parent
      case s"cd $directoryName" => currentDir = currentDir.getChild(directoryName)
      case "ls" =>
      case s"dir $directoryName" => addFileOrDir(directoryName, 0, true, currentDir)
      case s"$filesize $filename" => addFileOrDir(filename, filesize.toInt, false, currentDir)
    }
  }

  def addFileOrDir(name: String, size: Int, isDir: Boolean, currentDir: File): Unit = {
    val newFile = new File(name, size, isDir)
    newFile.parent = currentDir
    currentDir.addChild(newFile)
    allFiles.addOne(newFile.getFullPath(), newFile)
  }

  def findSizeOfSmallestDirectoryThatFreesUpEnoughSpace(): Int = {
    val totalUsedSpace = allFiles.get("/").get.getSize()
    val freeSpace = 70000000 - totalUsedSpace
    val spaceToBeFreedUp = 30000000 - freeSpace
    findFilesWithMinimumCertainSize(spaceToBeFreedUp).sortWith(_ < _)(0)
  }

  def findFilesWithAtMostCertainSize(limit: Int): List[Int] = {
    val onlyDirectoriesWithAtMostCertainSize = allFiles.filter(elem => elem._2.isDirectory && elem._2.getSize() < limit).values
    onlyDirectoriesWithAtMostCertainSize.map(_.getSize()).toList
  }

  def findFilesWithMinimumCertainSize(limit: Int): List[Int] = {
    val onlyDirectoriesWithAtMostCertainSize = allFiles.filter(elem => elem._2.isDirectory && elem._2.getSize() > limit).values
    onlyDirectoriesWithAtMostCertainSize.map(_.getSize()).toList
  }
}

class File(val name: String, val size: Int, val isDirectory: Boolean) {

  var children: List[File] = List()
  var parent: File = null

  def addChild(newChild: File): Unit = {
