@main def dayNine2022: Unit = {
  val initTime = System.currentTimeMillis
  println("Welcome to day nine of the Advent of Code!")
  val bridge = new Bridge()
  input.foreach(bridge.doStep(_))
  val visitedPositions = bridge.tail.visitedPositions.size
  println("The amount of visited positions is " + visitedPositions)
  val snappedBridge = new SnappedBridge()
  input.foreach(snappedBridge.doStep(_))
  val snappedBridgePositions = snappedBridge.allKnots(9).visitedPositions.size
  println("The amount of visited positions in the snapped bridge is " + snappedBridgePositions)
...
class Bridge {
  ...
  def doStep(step: Tuple2[Char, Int]): Unit = {
    head.move(step)
    tail.follow(head)
  }
}
...
class SnappedBridge {
  val allKnots: List[Knot] = List(Knot(1,1), Knot(1,1), Knot(1,1), Knot(1,1), Knot(1,1), Knot(1,1), Knot(1,1), Knot(1,1), Knot(1,1), Knot(1,1))

  def doStep(step: Tuple2[Char, Int]): Unit = {
    allKnots.head.move(step)
    val xHead = allKnots.head.xPosition
    val yHead = allKnots.head.yPosition
    while(!allKnots(1).isTouching(xHead, yHead)) {
      for(i <- 1 to 9) {
        letTailFollow(i, allKnots(i-1).xPosition, allKnots(i-1).yPosition)
      }
    }
  }

  def letTailFollow(index: Int, xFollowing: Int, yFollowing: Int): Unit = {
    if (!allKnots(index).isTouching(xFollowing, yFollowing)) {
      allKnots(index).followOneStep(allKnots(index-1))
      val x1 = allKnots(index).xPosition
      val y1 = allKnots(index).xPosition
    }
  }
}

class Knot(var xPosition: Int, var yPosition: Int) {

  var visitedPositions: Set[Tuple2[Int, Int]] = Set((1,1))

  def move(step: Tuple2[Char, Int]): Unit = {
    val direction = step._1
    val steps = step._2
    direction match {
      case 'R' => xPosition += steps
      case 'L' => xPosition -= steps
      case 'U' => yPosition += steps
      case 'D' => yPosition -= steps
      case _ => println("Wrong input!!")
    }
  }

  def moveStep(xStep: Int, yStep: Int): Unit = {
    xPosition += xStep
    yPosition += yStep
  }

  def followOneStep(head: Knot): Unit = {
    val xHead = head.xPosition
    val yHead = head.yPosition
    val xDirection = toDirection(xHead - xPosition)
    val yDirection = toDirection(yHead - yPosition)
    this.moveStep(xDirection, yDirection)
    visitedPositions = visitedPositions + ((this.xPosition, this.yPosition))
  }

  def follow(head: Knot): Unit = {
    val xHead = head.xPosition
    val yHead = head.yPosition
    while (!isTouching(xHead, yHead)) {
      val xDirection = toDirection(xHead - xPosition)
      val yDirection = toDirection(yHead - yPosition)
      this.moveStep(xDirection, yDirection)
      visitedPositions = visitedPositions + ((this.xPosition, this.yPosition))
